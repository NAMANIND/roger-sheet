...
export function ProcessorsList({ initialProcessors, onReload }: ProcessorsListProps) {
  const router = useRouter();
  const [processors, setProcessors] = useState(initialProcessors);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [processorToDelete, setProcessorToDelete] = useState<string | null>(null);
            ...
            <Card key={processor.name}>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {testingProcessor === processor.name ? 'Testing...' : 'Test'}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
              ...
              <CardContent>
                <div className="space-y-4">
                  {processor.type === 'script' && 'script' in processor.config && (
                    <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex gap-4">
                        <div>
                          ...
                          <Badge variant="outline">{processor.config.method}</Badge>
                        </div>
                        <div className="flex-1">
                          <h4 className="text-sm font-medium">URL</h4>
                          <code className="text-sm">{processor.config.url}</code>
                        </div>
                      </div>
                      {processor.config.headers && (
                        <div>
                          ...
                          <pre className="bg-muted p-2 rounded text-xs">
                            {JSON.stringify(processor.config.headers, null, 2)}
                          </pre>
                        </div>
                      )}
                    </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete processor &quot;{processorToDelete}&quot;?
              Jobs using this processor will fail.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
